...
int main() {
  ...
  unsigned long long sum = 0;  // utilizamos o tipo de dados unsigned long long
  ...
  while (i <= num) {
    ...
    product = product * i;
    i = i + 1;
    printf("soma: %llu \n", sum);  // utilizamos o especificador de formato %llu
                                   // para imprimir valores unsigned long long
    printf("produto: %llu \n", product);
  }
  return 0;
}